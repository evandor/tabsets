...
describe('MarkTabsetAsArchivedCommand', () => {
  ...
  it('mark tabset as favorite', async () => {
    ...
    const favoriteCmd = new MarkTabsetAsArchivedCommand(ts.id)
    const res: any = await favoriteCmd.execute()
    console.log("res", res)
    expect(res.message).toBe("The tabset has been archived. Check the settings page to revert this decision")
    expect(res.undoCommand).not.toBe(null)
    expect(ts.status).toBe("ARCHIVED")
  })
