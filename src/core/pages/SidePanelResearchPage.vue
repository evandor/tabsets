    ...
    <div class="column fitpage q-pa-sm q-mx-sm q-mt-md">
      <div class="col" style="max-width: 100%">
        ...
        <div class="text-subtitle-1 q-ma-xs text-bold" v-if="source">
          <PanelTabListElementWidget :key="'ptlew__' + source?.id" :tab="source" />
        </div>
...
import SourcePageAnnotation from 'src/core/pages/helper/SourcePageAnnotation.vue'
...
const updateBlobs = () => {
  if (source.value?.id) {
    useSnapshotsService()
      .getMetadataFor(source.value.id)
