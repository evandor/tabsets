<template>
  ...
  <q-list separator>
    <vue-draggable-next
      ...
      <q-item v-if="props.entities.length === 0 &&
                      inBexMode() &&
      ...
      <q-item
        ...
        class="q-my-lg"
        @click.stop="showDetails(entity)"
        @mouseover="showButtons(  entity.id,true)"
        @mouseleave="showButtons( entity.id, false)"
        @dragstart="startDrag($event, tab)"
        :key="props.group + '_' + entity.id">

        <CollectionListElementWidget :showButtons="showButtonsProp.get(entity.id)"
                                     :key="props.group + '__' + entity.id"
                                     :type="props.type"
                                     :collectionId="props.collectionId"
                                     :entity="entityAsEntity(entity)"
        />
...
import CollectionListElementWidget from "components/widgets/CollectionListElementWidget.vue";
...
const props = defineProps({
  entities: {type: Array as PropType<Array<Entity>>, required: true},
...
const entityAsEntity = (entity: Entity): Entity => entity as unknown as Entity
