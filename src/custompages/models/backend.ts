...
export class Page {
  ...
  constructor(id: string, name: string, version: number,
              created: number, changed: number, published: number,
              elements: ContentBlock[], redirectTo?: string) {
    this.id = id
    this.name = name;
...
export class Flex {
  ...
  constructor(container: string, direction: string, justifyContent: string, wrap: string, alignItems: string, alignContent: string,
              padding?: string,
              margin?: string,
              border?: string,
              maxWidth?: string,
...
export class Website {
  ...
  navigationIsDirty(): boolean {
    ...
    return JSON.stringify(_.map(this.navigation, n => n.originalOrder)) !== JSON.stringify(_.map(this.navigation, n => n.currentOrder))
  }
