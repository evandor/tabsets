...
watchEffect(() => {
  tabset.value = useTabsetsStore().getTabset('bibbly:news') || new Tabset(uid(), 'empty2', [])
  console.log('got tabset', tabsetId.value, tabset.value)
  tabsetFolder.value = tabset.value

  // done multiple times in sub-components, todo!
  tabset.value.tabs = useTagsService()
    .getTabsWithClassification('bibbly:news')
    .map((iT: IndexedTab) => {
      return iT.tab
    })
