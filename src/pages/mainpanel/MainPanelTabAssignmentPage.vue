<template>
  ...
  <q-toolbar class="text-primary">
    <q-toolbar-title> Tab Assignment </q-toolbar-title>
  </q-toolbar>
  ...
  <q-form @submit.prevent="setMatcher()" ref="theForm">
    <div class="q-pa-md q-gutter-sm">
      ...
      <div class="row items-baseline q-ma-md">
        <div class="col-3 text-bold">Original URL</div>
        <div class="col-9">
          {{ tab?.url }}
        </div>
      </div>
      ...
      <div class="row items-baseline q-ma-md">
        <div class="col-3 text-bold">Match when: starts with</div>
        <div class="col-9">
          <q-input
            v-model="matcher"
...
const removeMatcher = () => {
  ...
  if (tab.value) {
    useCommandExecutor()
      .execute(new TabAssignmentCommand(tab.value, tabsetId.value || '', undefined))
      .then((res) => {
        setTimeout(() => window.close(), 500)
      })
      .catch((err) => {
