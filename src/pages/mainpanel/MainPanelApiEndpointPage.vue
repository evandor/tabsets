...
<template>
  ...
  <q-page>
    ...
    <div class="q-ma-md">
      <div class="text-h6">MainPanelApiEndpointPage</div>

      ApiId {{ apiId }}<br>
      EndpointID {{ endpointId }}
      <q-btn v-if="endpointId" class="q-ml-md" size="xs" label="Delete" @click="deleteItem()"/>

      <br><br>
      Base URL: {{ api?.setup?.url }}

      <br><br>
      Base URL: {{ api }}

      <Vueform ref="form" :endpoint="submit" @change="formChange()">
        <TextElement name="endpoint"/>

        <GroupElement class="q-ma-lg" style="border:1px solid grey"
                      name="header"
                      label="Params">

          <GroupElement name="headersContainer" v-for="p in params">
          ...
          <!--          <TextElement name="key" label="X-RapidAPI-Key" default="test"/>-->
          <!--          <TextElement name="host" label="X-RapidAPI-Host" apidefault="test"/>-->
          ...
          <GroupElement name="container4">
            ...
            <GroupElement name="column2" :columns="{     container: 6   }">
              <TextElement name="newParamKey" label="Key"/>
            </GroupElement>
        ...
        <ButtonElement name="submit" submits align="right">submit</ButtonElement>
...
watchEffect(async () => {
  if (apiId.value && useApisStore().updated) {
    ...
    if (api.value) {
      const item: object | undefined = endpointId.value ? _.find(api.value!.items, (i: object) => i['id' as keyof object] === endpointId.value) : undefined
      console.log("hier", item)
      console.log("entity", api.value)
      const scheme: object = {}
...
const formChange = () => {
  ...
  const update = {}
  if (api.value && form.value && form.value.data) {
    formdata.value = form.value.data
    for (const formula of _.filter(api.value.fields, f => f.type === "formula")) {
      ...
      update[formula.name] = calculate(api.value, formula)
    }
    for (const sub of _.filter(api.value.fields, f => f.type === "substitute")) {
      console.log("found substitute", sub)
      //  calculatedField.value.set(formula.id, calculate(entity.value, formula))
      //form.value.data[formula.name] = "***" //calculate(entity.value, formula)
      update[sub.name] = substitute(api.value, sub)
    }
...
const deleteItem = () => {
  if (api.value) {
    console.log("deleting item id", endpointId.value)
    _.remove(api.value.items, {
      id: endpointId.value
    });
    //entity.value.items = removed
    console.log("got", api.value)
    sendMsg('entity-changed', api.value)
    window.close()
  }
}
const addParam = () => {
  if (form.value) {
    params.value.push({
      name: form.value.data.newParamKey,
      default: '',
    })
  }
  //
...
*, :after, :before, :root {
  ...
  --vf-bg-selected: rgba(17, 24, 39, .05);
  ...
  --vf-shadow-handles-hover: 0px 0px 0px 9px rgba(0, 0, 0, .15);
  --vf-shadow-handles-focus: 0px 0px 0px 9px rgba(0, 0, 0, .15);
  --vf-shadow-btn: 0px 3px 1px -2px rgba(0, 0, 0, .2), 0px 2px 2px 0px rgba(0, 0, 0, .14), 0px 1px 5px 0px rgba(0, 0, 0, .12);
  --vf-shadow-dropdown: 0 4px 6px 0 rgba(32, 33, 36, .28);
...
.dark, .dark *, .dark :after, .dark :before {
...
form [multiple]:focus, form [type=checkbox]:focus, form [type=date]:focus, form [type=datetime-local]:focus, form [type=email]:focus, form [type=month]:focus, form [type=number]:focus, form [type=password]:focus, form [type=radio]:focus, form [type=search]:focus, form [type=tel]:focus, form [type=text]:focus, form [type=time]:focus, form [type=url]:focus, form [type=week]:focus, form select:focus, form textarea:focus {
  outline: none
}
...
.vf-input-group .vf-input-field:-webkit-autofill, .vf-input-group .vf-input-field:-webkit-autofill:active, .vf-input-group .vf-input-field:-webkit-autofill:focus, .vf-input-group .vf-input-field:-webkit-autofill:hover {
...
.vf-input-group.vf-input-group-success .vf-input-field:-webkit-autofill, .vf-input-group.vf-input-group-success .vf-input-field:-webkit-autofill:active, .vf-input-group.vf-input-group-success .vf-input-field:-webkit-autofill:focus, .vf-input-group.vf-input-group-success .vf-input-field:-webkit-autofill:hover {
...
.vf-input-group.vf-input-group-danger .vf-input-field:-webkit-autofill, .vf-input-group.vf-input-group-danger .vf-input-field:-webkit-autofill:active, .vf-input-group.vf-input-group-danger .vf-input-field:-webkit-autofill:focus, .vf-input-group.vf-input-group-danger .vf-input-field:-webkit-autofill:hover {
...
.vf-floating-wrapper ~ .vf-input, .vf-floating-wrapper ~ div .vf-input, .vf-input-group.vf-input-group .vf-floating-wrapper ~ .vf-input-field, .vf-input-group.vf-input-group .vf-floating-wrapper ~ div .vf-input-field {
  padding-bottom: calc(var(--vf-py-input) - var(--vf-floating-top) / 2);
  padding-top: calc(var(--vf-py-input) + var(--vf-floating-top) / 2)
}

.vf-floating-wrapper ~ .vf-input-sm, .vf-floating-wrapper ~ div .vf-input-sm, .vf-input-group.vf-input-group-sm .vf-floating-wrapper ~ .vf-input-field, .vf-input-group.vf-input-group-sm .vf-floating-wrapper ~ div .vf-input-field {
...
.vf-floating-wrapper ~ .vf-input-lg, .vf-floating-wrapper ~ div .vf-input-lg, .vf-input-group.vf-input-group-lg .vf-floating-wrapper ~ .vf-input-field, .vf-input-group.vf-input-group-lg .vf-floating-wrapper ~ div .vf-input-field {
...
.vf-radio-container, .vf-radio-wrapper {
  align-items: flex-start;
  display: flex;
  width: 100%
}
...
.vf-btn.vf-btn-disabled, .vf-btn[disabled] {
  cursor: not-allowed;
  opacity: .6
}
...
body:not([dir=rtl]) .vf-checkbox-wrapper-right, body:not([dir=rtl]) .vf-radio-wrapper-right {
...
body:not([dir=rtl]) .vf-checkbox-text-right, body:not([dir=rtl]) .vf-radio-text-right {
  order: -1
}
...
body[dir=rtl] .vf-checkbox-wrapper-left, body[dir=rtl] .vf-radio-wrapper-left {
  justify-content: flex-end
}
...
body[dir=rtl] .vf-checkbox-text-left, body[dir=rtl] .vf-radio-text-left {
  order: -1
}
...
.vf-text-type .vf-xs\:vf-vertical-label, :not(.vf-text-type) .vf-xs\:vf-vertical-label {
  padding-bottom: calc(var(--vf-gutter) / 3);
  padding-right: 0;
  padding-top: 0
}

.vf-text-type .vf-xs\:vf-vertical-label-sm, :not(.vf-text-type) .vf-xs\:vf-vertical-label-sm {
  padding-bottom: calc(var(--vf-gutter-sm) / 3);
  padding-right: 0;
  padding-top: 0
}

.vf-text-type .vf-xs\:vf-vertical-label-lg, :not(.vf-text-type) .vf-xs\:vf-vertical-label-lg {
  padding-bottom: calc(var(--vf-gutter-lg) / 3);
  padding-right: 0;
  padding-top: 0
}

.vf-xs\:vf-horizontal-label, .vf-xs\:vf-horizontal-label-lg, .vf-xs\:vf-horizontal-label-sm {
  padding-bottom: 0
}
...
@media (min-width: 640px) {
  .vf-text-type .vf-sm\:vf-vertical-label, :not(.vf-text-type) .vf-sm\:vf-vertical-label {
    padding-bottom: calc(var(--vf-gutter) / 3);
    padding-right: 0;
    padding-top: 0
  }

  .vf-text-type .vf-sm\:vf-vertical-label-sm, :not(.vf-text-type) .vf-sm\:vf-vertical-label-sm {
    padding-bottom: calc(var(--vf-gutter-sm) / 3);
    padding-right: 0;
    padding-top: 0
  }

  .vf-text-type .vf-sm\:vf-vertical-label-lg, :not(.vf-text-type) .vf-sm\:vf-vertical-label-lg {
    padding-bottom: calc(var(--vf-gutter-lg) / 3);
    padding-right: 0;
    padding-top: 0
  }

  .vf-sm\:vf-horizontal-label, .vf-sm\:vf-horizontal-label-lg, .vf-sm\:vf-horizontal-label-sm {
    padding-bottom: 0
  }
...
@media (min-width: 768px) {
  .vf-text-type .vf-md\:vf-vertical-label, :not(.vf-text-type) .vf-md\:vf-vertical-label {
    padding-bottom: calc(var(--vf-gutter) / 3);
    padding-right: 0;
    padding-top: 0
  }

  .vf-text-type .vf-md\:vf-vertical-label-sm, :not(.vf-text-type) .vf-md\:vf-vertical-label-sm {
    padding-bottom: calc(var(--vf-gutter-sm) / 3);
    padding-right: 0;
    padding-top: 0
  }

  .vf-text-type .vf-md\:vf-vertical-label-lg, :not(.vf-text-type) .vf-md\:vf-vertical-label-lg {
    padding-bottom: calc(var(--vf-gutter-lg) / 3);
    padding-right: 0;
    padding-top: 0
  }

  .vf-md\:vf-horizontal-label, .vf-md\:vf-horizontal-label-lg, .vf-md\:vf-horizontal-label-sm {
    padding-bottom: 0
  }
...
@media (min-width: 1024px) {
  .vf-text-type .vf-lg\:vf-vertical-label, :not(.vf-text-type) .vf-lg\:vf-vertical-label {
    padding-bottom: calc(var(--vf-gutter) / 3);
    padding-right: 0;
    padding-top: 0
  }

  .vf-text-type .vf-lg\:vf-vertical-label-sm, :not(.vf-text-type) .vf-lg\:vf-vertical-label-sm {
    padding-bottom: calc(var(--vf-gutter-sm) / 3);
    padding-right: 0;
    padding-top: 0
  }

  .vf-text-type .vf-lg\:vf-vertical-label-lg, :not(.vf-text-type) .vf-lg\:vf-vertical-label-lg {
    padding-bottom: calc(var(--vf-gutter-lg) / 3);
    padding-right: 0;
    padding-top: 0
  }

  .vf-lg\:vf-horizontal-label, .vf-lg\:vf-horizontal-label-lg, .vf-lg\:vf-horizontal-label-sm {
    padding-bottom: 0
  }
...
@media (min-width: 1280px) {
  .vf-text-type .vf-xl\:vf-vertical-label, :not(.vf-text-type) .vf-xl\:vf-vertical-label {
    padding-bottom: calc(var(--vf-gutter) / 3);
    padding-right: 0;
    padding-top: 0
  }

  .vf-text-type .vf-xl\:vf-vertical-label-sm, :not(.vf-text-type) .vf-xl\:vf-vertical-label-sm {
    padding-bottom: calc(var(--vf-gutter-sm) / 3);
    padding-right: 0;
    padding-top: 0
  }

  .vf-text-type .vf-xl\:vf-vertical-label-lg, :not(.vf-text-type) .vf-xl\:vf-vertical-label-lg {
    padding-bottom: calc(var(--vf-gutter-lg) / 3);
    padding-right: 0;
    padding-top: 0
  }

  .vf-xl\:vf-horizontal-label, .vf-xl\:vf-horizontal-label-lg, .vf-xl\:vf-horizontal-label-sm {
    padding-bottom: 0
  }
...
@media (min-width: 1536px) {
  .vf-text-type .vf-2xl\:vf-vertical-label, :not(.vf-text-type) .vf-2xl\:vf-vertical-label {
    padding-bottom: calc(var(--vf-gutter) / 3);
    padding-right: 0;
    padding-top: 0
  }

  .vf-text-type .vf-2xl\:vf-vertical-label-sm, :not(.vf-text-type) .vf-2xl\:vf-vertical-label-sm {
    padding-bottom: calc(var(--vf-gutter-sm) / 3);
    padding-right: 0;
    padding-top: 0
  }

  .vf-text-type .vf-2xl\:vf-vertical-label-lg, :not(.vf-text-type) .vf-2xl\:vf-vertical-label-lg {
    padding-bottom: calc(var(--vf-gutter-lg) / 3);
    padding-right: 0;
    padding-top: 0
  }

  .vf-2xl\:vf-horizontal-label, .vf-2xl\:vf-horizontal-label-lg, .vf-2xl\:vf-horizontal-label-sm {
    padding-bottom: 0
  }
